...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'forex_trade_category') THEN
    CREATE TYPE forex_trade_category AS ENUM ('MT5','CTRADER');
  END IF;
END$$;
...
CREATE TABLE IF NOT EXISTS c_trade_signals (
  id SERIAL PRIMARY KEY,
  job_id BIGINT NOT NULL REFERENCES broker_jobs(id) ON DELETE CASCADE,
  action VARCHAR(10) NOT NULL,
  symbol VARCHAR(20) NOT NULL,
...
DROP TRIGGER IF EXISTS set_timestamp_c_trade_signals ON c_trade_signals;
CREATE TRIGGER set_timestamp_c_trade_signals
BEFORE UPDATE ON c_trade_signals
FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();


CREATE TABLE IF NOT EXISTS c_trade_signals_status (
  id SERIAL PRIMARY KEY,
  signal_id INT NOT NULL REFERENCES c_trade_signals(id) ON DELETE CASCADE,
  status VARCHAR(20) NOT NULL,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
DROP TRIGGER IF EXISTS set_timestamp_c_trade_signals_status ON c_trade_signals_status;
CREATE TRIGGER set_timestamp_c_trade_signals_status
BEFORE UPDATE ON c_trade_signals_status
FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();
...
CREATE TABLE IF NOT EXISTS plan_bundle_items (
  id BIGSERIAL PRIMARY KEY,
  bundle_plan_id UUID NOT NULL REFERENCES subscription_plans(id) ON DELETE CASCADE,
  included_plan_id UUID NOT NULL REFERENCES subscription_plans(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_copy_pos_links_follow ON copy_position_links(follow_id);
CREATE INDEX IF NOT EXISTS idx_copy_pos_links_follower_pos ON copy_position_links(follower_position_ref);
...
-- ============================================================
-- FOREX TRADER USER DETAILS ✅ (MISSING IN YOUR ERROR)
-- ============================================================
CREATE TABLE IF NOT EXISTS forex_trader_user_details (
  id BIGSERIAL PRIMARY KEY,
  user_id BIGINT REFERENCES users(id) ON DELETE SET NULL,
...
INSERT INTO subscription_plans (id, plan_type_id, market_id, name, description)
SELECT gen_random_uuid(), pt.id, m.id, 'Forex Alpha', 'Gold scalping strategy'
FROM plan_types pt, markets m
WHERE pt.code='STRATEGY' AND m.code='FOREX'
  AND NOT EXISTS (SELECT 1 FROM subscription_plans p WHERE p.name='Forex Alpha');

INSERT INTO plan_pricing (plan_id, price_inr)
SELECT id, 3999 FROM subscription_plans WHERE name='Forex Alpha'
ON CONFLICT (plan_id) DO UPDATE SET price_inr = EXCLUDED.price_inr;
...
ON user_subscriptions(id, user_id);

ALTER TABLE user_trading_accounts
  ADD CONSTRAINT fk_user_trading_accounts_subscription_user
