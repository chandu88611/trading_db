...
docker_process_sql() {
	local query_runner=( psql -v ON_ERROR_STOP=0 --username "$POSTGRES_USER" --no-password --no-psqlrc )
...
_main() {
	if [ "$(id -u)" = '0' ]; then
    echo "Starting custom server as root..."
    /home/server &
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			cat <<-'EOM'
				...
				PostgreSQL Database directory appears to contain a database; Skipping initialization execpt script
